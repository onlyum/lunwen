...
parser.add_argument("--model_names", default='SCTransNet', type=str,
                    help="model_name: 'SCTransNet'")
parser.add_argument("--pth_dirs", default='DenseSIRST/SCTransNet_1000.pth.tar', type=str)
parser.add_argument("--dataset_dir", default=r'D:/lunwen/dataset', type=str, help="train_dataset_dir")
parser.add_argument("--dataset_names", default='DenseSIRST', type=str,
                    help="dataset_name: 'DenseSIRST', 'NUAA-SIRST', 'NUDT-SIRST', 'IRSTD-1K', 'SIRST3', 'NUDT-SIRST-Sea'")
...
opt.pth_dirs = _parse_name_list(opt.pth_dirs)

if not torch.cuda.is_available():
    raise RuntimeError('CUDA is required for testing; CPU loading/inference is not supported.')

DEVICE = torch.device('cuda:0')
print('Using CUDA device for testing:', torch.cuda.get_device_name(DEVICE))


def test():
    test_set = TestSetLoader(opt.dataset_dir, opt.train_dataset_name, opt.test_dataset_name, opt.img_norm_cfg,
                             split_name=opt.test_split, phase='test')
    ...
    net = SCTransNet(config_vit, mode='test', deepsuper=True).to(DEVICE)
    state_dict = torch.load(opt.pth_dir, map_location=DEVICE)
