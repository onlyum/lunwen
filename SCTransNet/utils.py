...
def _find_dense_sirst_root(dataset_name, dataset_dir):
    candidates = [
    ...
    for candidate in candidates:
        ...
        if _is_dense_sirst_root(candidate):
            return candidate
    return None


def _open_norm_image(img_dir, img_pth):
    for suffix in ['.png', '.jpg', '.bmp']:
        path = os.path.join(img_dir, img_pth + suffix)
        if os.path.exists(path):
            return Image.open(path).convert('I')
    raise FileNotFoundError(f'Cannot find image {img_pth} in {img_dir}')


def _calc_img_norm_cfg(img_dir, img_list):
    mean_list = []
    std_list = []
    for img_pth in img_list:
        img = np.array(_open_norm_image(img_dir, img_pth), dtype=np.float32)
        mean_list.append(img.mean())
        std_list.append(img.std())
    img_norm_cfg = dict(mean=float(np.array(mean_list).mean()), std=float(np.array(std_list).mean()))
    if img_norm_cfg['std'] == 0:
        img_norm_cfg['std'] = 1.0
    return img_norm_cfg


def get_img_norm_cfg(dataset_name, dataset_dir, train_split='train_v2'):
    cache_key = (dataset_name, os.path.abspath(dataset_dir), train_split)
    if cache_key in _IMG_NORM_CACHE:
        return _IMG_NORM_CACHE[cache_key]
    ...
    elif dataset_name == 'DenseSIRST':
        dense_root = _find_dense_sirst_root(dataset_name, dataset_dir)
        if dense_root is None:
            raise FileNotFoundError('Cannot find DenseSIRST root with PNGImages/ and Splits/.')
        split_name = train_split if train_split.endswith('.txt') else train_split + '.txt'
        with open(os.path.join(dense_root, 'Splits', split_name), 'r') as f:
            train_list = [line.strip() for line in f.read().splitlines() if line.strip()]
        img_norm_cfg = _calc_img_norm_cfg(os.path.join(dense_root, 'PNGImages'), train_list)
    else:
        with open(dataset_dir + '/' + dataset_name + '/img_idx/train_' + dataset_name + '.txt', 'r') as f:
            train_list = [line.strip() for line in f.read().splitlines() if line.strip()]
        img_dir = dataset_dir + '/' + dataset_name + '/images/'
        img_norm_cfg = _calc_img_norm_cfg(img_dir, train_list)
    _IMG_NORM_CACHE[cache_key] = img_norm_cfg
    return img_norm_cfg
