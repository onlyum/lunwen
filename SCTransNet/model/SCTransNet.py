...
class InfraredPrior(nn.Module):
    ...
    def forward(self, x):
        ...
        high_freq = torch.abs(x - F.avg_pool2d(x, kernel_size=3, stride=1, padding=1))
        grad_x = F.conv2d(x, self.sobel_x, padding=1)
        grad_y = F.conv2d(x, self.sobel_y, padding=1)
        gradient = torch.sqrt(grad_x * grad_x + grad_y * grad_y + 1e-6)
        prior = self._norm_map(contrast) + self._norm_map(high_freq) + self._norm_map(gradient)
        return self._norm_map(prior)


class SCTransNet(nn.Module):
    def __init__(self, config, n_channels=1, n_classes=1, img_size=256, vis=False, mode='train',
                 deepsuper=True, use_prior=False, prior_gamma_init=0.0, use_aux=False):
        super().__init__()
        self.vis = vis
        ...
        if self.use_prior:
            self.prior = InfraredPrior()
            self.prior_gamma = nn.Parameter(torch.tensor(float(prior_gamma_init), dtype=torch.float32))
        ...
        if self.use_aux:
            ...
            self.edge_head = nn.Conv2d(in_channels, 1, kernel_size=1)
    ...
    def _apply_prior(self, feat, prior):
        if not self.use_prior or prior is None:
            return feat
        prior = F.interpolate(prior, size=feat.shape[-2:], mode='bilinear', align_corners=False)
        return feat * (1.0 + self.prior_gamma * prior)

    def _with_aux(self, seg_outputs, decoder_feat):
        if not self.use_aux:
            return seg_outputs
        return {
            'seg': seg_outputs,
            'center': torch.sigmoid(self.center_head(decoder_feat)),
            'edge': torch.sigmoid(self.edge_head(decoder_feat)),
        }

    def forward(self, x):
        prior = self.prior(x) if self.use_prior else None
        x1 = self.inc(x)  # 64 224 224
        x1 = self._apply_prior(x1, prior)
        x2 = self.down_encoder1(self.pool(x1))  # 128 112 112
        x2 = self._apply_prior(x2, prior)
        x3 = self.down_encoder2(self.pool(x2))  # 256 56  56
        x3 = self._apply_prior(x3, prior)
        x4 = self.down_encoder3(self.pool(x3))  # 512 28  28
        x4 = self._apply_prior(x4, prior)
        d5 = self.down_encoder4(self.pool(x4))  # 512 14  14
        d5 = self._apply_prior(d5, prior)
        #  CCT
        f1 = x1
        f2 = x2
        f3 = x3
        ...
        if self.deepsuper:
            ...
            if self.mode == 'train':
                ...
                return self._with_aux(seg_outputs, d1)
            else:
                return torch.sigmoid(out)
        else:
            seg_output = torch.sigmoid(out)
            if self.mode == 'train':
                return self._with_aux(seg_output, d1)
            return seg_output
